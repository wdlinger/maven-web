...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter{
	...
	@Override
	//configure(HttpSecurity http)方法.antMatchers("/login","/css/**","/image/*").permitAll()
    protected void configure(HttpSecurity http) throws Exception {
		//通过authorizeRequests()定义哪些URL需要被保护、哪些不需要被保护。
		//例如以上代码指定了/和/home不需要任何认证就可以访问，其他的路径都必须通过身份验证。
        http
            .authorizeRequests()
                .antMatchers("/login", "/resources/**","/register","/user/register","/common/**").permitAll()
